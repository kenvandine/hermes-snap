...
Snapcraft will install `hermes-agent` from PyPI via the Python plugin, install the `playwright` Python package, bake a pinned Chromium revision for browser automation, and bundle Node.js 22 for runtime browser tool invocation. It builds for both `amd64` and `arm64` (e.g. Raspberry Pi 5).

## Installing

Build natively on the target architecture, then install the matching snap:

```
sudo snap install --classic hermes-agent_<version>_amd64.snap --dangerous   # x86_64
sudo snap install --classic hermes-agent_<version>_arm64.snap --dangerous   # ARM64 (Raspberry Pi 5)
```
...
**Bundled Node.js** — A separate `nodejs` part stages Node.js 22 via the `npm` plugin (no npm packages installed — just the runtime). This allows hermes to use Node.js for browser automation tools at runtime without requiring it to be installed on the host. The Node binary is `patchelf`'d to core24's dynamic linker, selecting the interpreter and library triplet to match the target architecture so it works on both amd64 and arm64.
